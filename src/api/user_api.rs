...
use mongodb::{
    bson::{RawDocumentBuf,extjson::de::Error, doc}, 
};
...
#[post("/disabled-quiz", data = "<new_user>")]
pub fn create(
    db: &State<MongoRepo>,
    new_user: Json<HashMap<&str, Value>>,
) -> Result<Json<InsertOneResult>, Status> {
    let mut map = HashMap::new();
    for  (k, v) in new_user.iter() {
        map.insert(k.to_owned(), v.to_owned());
    }
    let user_detail = db.create(map);
    match user_detail {
...
#[get("/disabled-quiz")]
pub fn get_list(db: &State<MongoRepo>) -> Result<Json<Vec<RawDocumentBuf>>, Status> {
    let items = db.get_list();
    match items {
